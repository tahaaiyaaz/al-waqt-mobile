...
app2.get('/getNearestMasjid', async (req, res) => {
    try {
        ...
        console.log("step 1 ")
        let { latitude, longitude, radiusInKm = 5 } = req.query;
        ...
        try {
            ...
            return res.status(400).send({ message: "Invalid latitude or longitude format" });
        }
        ...
        snapshots.forEach(snapshot => {
            snapshot.forEach(doc => {
                let masjid = doc.data();
                let distanceInMeters = geofire.distanceBetween(
                    center,
        ...
        res.status(200).send({ message: "Nearby masjids fetched successfully", masjids: matchingMasjids });
    } catch (error) {
...
app2.post("/addmasjid", async (req, res) => {
    ...
    try {
        ...
        const { latitude, longitude, countryName, cityName, stateName, details } = req.body;

        if (!latitude || !longitude || !countryName || !cityName || !stateName || !details) {
        ...
        res.status(201).send({ message: "Masjid added successfully", masjidId: docRef.id });
    } catch (error) {
...
app2.put("/updateMasjid", async (req, res) => {
    ...
    try {
        ...
        if (!masjidId || !updates) {
            return res.status(400).send({ message: "Missing or invalid masjidId or updates" });
        }
...
app2.post('/login', async (req, res) => {
    ...
    try {
        ...
        const userCredential = await bc.signInWithEmailAndPassword(auth, req.body.email, req.body.password)
...
app2.post('/signup', async (req, res) => {
    ...
    try {
        ...
        const userCredential = await bc.createUserWithEmailAndPassword(auth, req.body.email, req.body.password)
...
let favoriteIds =[]
        ...
        if (!favoriteIds || !Array.isArray(favoriteIds) || favoriteIds.length === 0) {
            return res.status(400).send({ message: "favoriteIds must be a non-empty array." });
        }
...
app2.post('/signupwithmobile', async (req, res) => {
    try {
        ...
        return res.status(400).send({ message: "Invalid Credentials", error: e.message });
    }
});
...
app2.put("/updateUser", async (req, res) => {
    try {
      ...
      if (!userId || !updates) {
        return res.status(400).send({ message: "Missing or invalid userId or updates" });
      }
